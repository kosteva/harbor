...
Stops all currently running services. When `dmr`, `mlx`, or `omlx` are running (or named explicitly), Harbor also stops their host-managed inference processes before tearing down Compose containers. Host backend stop failures are logged but do not block the Compose shutdown.

```bash
# Stop all services (including host backends for active dmr/mlx/omlx)
harbor down

# Stop a specific service and its compose companions
harbor down beszel
...
| `hf.ssl.cert.file` | Host path to a custom CA bundle for `harbor hf` (corporate TLS inspection); mounted at `/etc/ssl/certs/custom-ca.pem` and passed to `SSL_CERT_FILE` / `REQUESTS_CA_BUNDLE` when set |
