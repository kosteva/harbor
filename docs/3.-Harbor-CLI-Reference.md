...
# With multiple services, tails logs of all of them
harbor up webui ollama -t
```
...
By default, Harbor will try to infer some capabilities of the host (and match related [cross files](./6.-Harbor-Compose-Setup.md#cross-service-file)), such as Nvidia GPU availability (`nvidia` capability) or presence of modern Docker Compose features (`mdc` capability).
...
With no arguments, stops and removes all currently running services.

When service names are given, Harbor stops and removes **only** those services and their `<service>-*` compose companions (for example `beszel-agent` for `beszel`). Every other running service keeps running - a scoped `harbor down` never sweeps unrelated containers.

When `dmr`, `mlx`, or `omlx` are running (or named explicitly), Harbor also stops their host-managed inference processes before tearing down Compose containers. Host backend stop failures are logged but do not block the Compose shutdown.

```bash
# Stop all services (including host backends for active dmr/mlx/omlx)
harbor down

# Stop only beszel and its compose companions - other services stay up
harbor down beszel

# Stop only the oMLX host server and proxy
harbor down omlx

# Remove named volumes as well
harbor down --volumes

# Scoped teardown: removes the containers' anonymous volumes only
harbor down beszel --volumes

# Remove images - only supported for a bare 'harbor down'
harbor down --rmi local
```

`--rmi` applies to the whole project, so it is only honoured by a bare `harbor down`; when services are named it is ignored with a warning.

`--volumes` also behaves differently between the two forms. A bare `harbor down --volumes` runs `compose down -v` and removes the project's named volumes. A scoped `harbor down <service> --volumes` runs `compose rm -v`, which only removes the anonymous volumes attached to the removed containers - named volumes declared in the compose files survive and are only removed by the bare form.
...
With no arguments, restarts every currently running service. With service names, Harbor recreates **only** those services (and their `<service>-*` companions) - other running services are left alone and are not recreated.

```bash
# Restart everything that is running
harbor restart

# Restart a specific service only - nothing else is touched
...
- Harbor service handle (e.g. `ollama`, `webui`, etc.) — runs `docker compose pull` for matching services
- Model spec (e.g. `gemma3n:e4b-it-q8_0`, `microsoft/Phi-3.5-mini-instruct-gguf:Q4_K_M`) — uses the same smart routing as [`harbor models pull`](#harbor-models-pull-model)

Flags such as `--no-defaults` are passed to the compose resolver. If any non-flag argument is not a known service handle, Harbor treats the invocation as a model pull: only the model arguments are processed, and any service names mixed in are skipped with a warning.
...
**Model routing** (when the argument is not a service handle):

1. Harbor queries the Hugging Face Hub API for `org/repo` (the part before an optional `:tag`).
2. If the repo is not found, the spec is passed to Ollama's `pull` command.
3. If the repo exists and publishes `.gguf` files, Harbor downloads via an ephemeral llama.cpp server into the llama.cpp cache.
4. If the repo exists but has no GGUF artifacts, Harbor downloads the repository via the Hugging Face CLI into the HF cache.

Use [`harbor models pull --source`](./3.-Harbor-CLI-Reference.md#harbor-models-pull-model) for explicit routing to Docker Model Runner, MLX, or oMLX. `harbor pull` does not accept `--source`.
...
### `harbor launch [launch-options] [--service] <service|tool> [args]`

Launches a Harbor service CLI or supported host coding tool with the currently running Harbor services included in the compose selection. This is the main workflow for pointing installed coding tools at Harbor backends without hand-editing each tool's provider config.

```bash
# Start a backend once
harbor up ollama

# Every argument after the tool/service name is passed through unchanged
harbor launch mi -p "say hello"

# Force container service mode when a name also has a host tool adapter
harbor launch --service opencode --help
```

For known Harbor service CLIs, `launch` dispatches to the same implementation as the direct command, such as `harbor promptfoo`, `harbor plandex`, or `harbor aider`. For other service handles it falls back to [`harbor run`](#harbor-run-service-command). Use `--service` before the handle to bypass host tool adapters and run a Harbor service container explicitly; this is useful for overlapping names such as `mi`, `opencode`, `openclaw`, and `hermes`, where `harbor launch mi` means the installed host mi CLI and `harbor launch --service mi` means the Harbor `mi` container service.

#### Host coding tools

`launch` can also run installed host tools against a running Harbor OpenAI-compatible backend:
...
# Route through Harbor Boost's codemode module: one execute_code tool, tools called from Python
harbor launch --codemode --backend ollama --model qwen3.5:4b codex
...
# Pass arguments to the host tool after the tool name
harbor launch --model qwen3.5:4b codex --sandbox workspace-write
```

Supported host tools are aligned with Ollama's launch targets: `claude`, `codex`, `copilot`, `droid`, `grok`, `hermes`, `mi`, `openclaw`, `opencode`, `pi`, `pool`, and `vscode`. Host tool launchers accept `--backend <service>`, `--model <model>`, `--config`, `--web`, `--codemode`, and `--workflow <module>` before the tool name. Supported OpenAI-compatible launch backends include `ollama`, `llamacpp`, `ikllamacpp`, `vllm`, `dmr`, `mlx`, `omlx`, `tabbyapi`, `mistralrs`, `sglang`, `lmdeploy`, `aphrodite`, `ktransformers`, and `unsloth-studio`. Everything after the tool name belongs to the tool, so a tool's own `--model`, `--backend`, or other flags are not intercepted by Harbor. The `--web` modifier starts the `boost` service with the selected target backend, configures a generated workflow named `boost-web`, and points the launched tool at the Boost model ID, such as `boost-web-qwen3.5:4b`, instead of the raw backend model. `--web` enables `web_search` and `read_url`, and starts SearXNG for the local web search provider. The `--workflow <module>` modifier instead routes the launched tool through a single named Boost module (any built-in or custom module, e.g. `quickhop` or `deephop` — SearXNG is auto-started for those two); `--codemode` is sugar for `--workflow codemode`, which replaces the whole Boost tool catalog with a single `execute_code` tool the model drives with Python programs; it cannot be combined with `--web` or with a different `--workflow`. `--web` and `--workflow` cannot be combined, and neither works with `claude` (Anthropic Messages API). Without an explicit backend, Harbor checks already-running OpenAI-compatible backend services — backends listed in `services.default` first, then the remaining supported backends — and uses the first reachable `/v1/models` endpoint; if none are running, it starts `llamacpp` as the default backend. Without an explicit model, Harbor reads every advertised model from OpenAI-style `data[]` responses and common backend list variants such as Ollama-style `models[]`, then chooses a non-embedding model as the launch default when one is available.

Different tools accept backend configuration differently. Harbor uses environment variables for `claude`, `codex`, `copilot`, `grok`, `hermes`, `mi`, `opencode`, and `pool`; writes the documented JSON config files for `droid`, `openclaw`, and `pi`; and opens VS Code through the `code` command. Config-capable tools are given every model advertised by the selected backend, while single-model CLIs still receive one selected model for the immediate command. Use `--config` to print the selected backend/model and write any config files without starting the target tool. For colliding Harbor service names such as `mi`, `opencode`, `openclaw`, and `hermes`, use `harbor launch --service <name>` to launch the container service instead of the host tool adapter.

Host-tool adapters execute from the directory where you invoked `harbor launch`, not from Harbor's installation directory. This preserves the project/workspace context for tools that inspect the current directory.

`harbor launch pi` runs Pi from the directory where you invoked Harbor and, unless you pass Pi session flags such as `--session-dir`, `--session`, `--resume`, or `--continue`, stores the session under a workspace-specific Pi session directory. This keeps Pi from resuming a Harbor checkout session when you launch it from another project directory.

Codex CLI currently uses the Responses API tool schema. With llama.cpp-family backends, Codex can fail after launch with `400 'type' of tool must be 'function'` when the backend rejects non-function tool payloads. Harbor prints this compatibility warning before invoking Codex; for live llama.cpp smoke checks, prefer `harbor launch --backend llamacpp opencode` or use Codex with a backend that accepts Codex's Responses API tool schema.

If no compatible backend is running, `launch` starts `llamacpp` and uses it when its `/v1/models` endpoint is reachable. If `--backend <service>` names a stopped backend, Harbor starts that backend before launching the tool. Empty `/v1/models` responses require a loaded model or an explicit `--model <model>`; invalid `/v1/models` responses are treated as endpoint compatibility failures. Passing `--model <model>` skips model discovery from `/v1/models`, but Harbor still verifies that the selected backend is reachable.

For a live end-user smoke test from a development checkout, start a backend and run the dev helper:

```bash
harbor up ollama
harbor dev launch-live-smoke --backend ollama --model qwen3.5:4b

# Validate generated adapter config without starting host tools
harbor dev launch-live-smoke --tools codex,opencode --backend llamacpp --config-only
```

The helper runs installed `codex`, `claude`, `mi`, and `opencode` CLIs through `harbor launch` in non-interactive prompt mode, skips missing tools by default, and fails if a selected launched tool exits non-zero or exceeds its timeout.
...
Runs a comprehensive diagnostic script to check if all requirements are met for Harbor to run properly. Returns a non-zero exit code when issues are found.

Checks performed:
- **Docker:** installation, daemon reachability (with 10s timeout), sudo-free access
- **Docker Compose:** version 2.23.1+ required
- **Disk space:** warns when Docker storage has less than 10GB free
- **Registry:** tests Docker Hub connectivity, reports proxy configuration, diagnoses DNS/network failures
- **Harbor files:** home directory, default profile, `.env` configuration, CLI symlink validity
- **GPU (when hardware is present):** NVIDIA GPU + Container Toolkit, AMD ROCm (`/dev/kfd`, render devices, kernel module)
- **WSL (when applicable):** WSL version (flags WSL1 as unsupported), Windows-mounted filesystem performance warnings
...
### `harbor ikllamacpp <command>`

Runs CLI tasks specific to managing `ikllamacpp`, Harbor's ik_llama.cpp server.

The subcommands mirror `harbor llamacpp`: `models`, `model`, `gguf`, `args`, and `build`.

```bash
harbor ikllamacpp models
harbor ikllamacpp model https://huggingface.co/user/repo/blob/main/file.gguf
harbor ikllamacpp gguf /models/model.gguf
harbor ikllamacpp args '-c 4096 -n 512'
...
Runs the Hugging Face Hub CLI (`hf`) in a container against the host's HuggingFace cache.
...
Cross-source model management for Ollama, HuggingFace Hub, llama.cpp caches, Docker Model Runner, MLX, and oMLX. By default, cache operations preserve the existing Ollama/HuggingFace/llama.cpp routing. Use `--source` when a model belongs to Docker Model Runner, MLX, or oMLX.

> Aliases: `harbor models list` = `harbor models ls`, `harbor models remove` = `harbor models rm`, `harbor models dmr pull ...` = `harbor models pull --source dmr ...`, `harbor models mlx pull ...` = `harbor models pull --source mlx ...`, `harbor models omlx pull ...` = `harbor models pull --source omlx ...`

#### `harbor models ls`

List locally available models across Ollama, HuggingFace Hub, the llama.cpp GGUF cache, Docker Model Runner, MLX, and oMLX. Host-backed DMR, MLX, and oMLX are queried by default only when their Harbor proxy service is running; use `--source dmr`, `--source mlx`, or `--source omlx` to query their host endpoints directly.

```bash
# List all models (table view)
harbor models ls
...
The command queries these sources in parallel:

- **ollama** — calls the running Ollama instance. Skipped if Ollama is not up.
- **hf** — scans the HuggingFace hub cache (`HARBOR_HF_CACHE/hub`). Only `model`-type repos are shown (datasets and spaces are excluded). For each repo, the latest revision is used. Individual GGUF files or safetensors shards are shown as indented sub-rows with sizes. GGUF metadata (architecture, parameter count, quantization, context length) is read from file headers.
- **llamacpp** — recursively walks `HARBOR_LLAMACPP_CACHE` for `.gguf` files. Multimodal projector files (`mmproj`) are excluded. The canonical `org/repo` identity is resolved from subdirectory paths, manifest filenames, or sidecar `.json` files. Model IDs are shown as `org/repo:quantTag`.
- **dmr** — calls Docker Model Runner through `HARBOR_DMR_UPSTREAM_URL`.
- **mlx** — calls the mlx-lm server through `HARBOR_MLX_UPSTREAM_URL`.
- **omlx** — calls the oMLX server through `HARBOR_OMLX_UPSTREAM_URL`.

**Sample output:**

```
SOURCE    MODEL                                              SIZE      DETAILS
ollama    nomic-embed-text:latest                            274 MB    nomic-bert 137M F16
ollama    mxbai-embed-large:latest                           670 MB    bert 334M F16
ollama    qwen3.5:9b                                         6.9 GB    qwen35 8.95B unknown
ollama    qwen3.5:0.8b                                       1.0 GB    qwen35 873.44M Q8_0
ollama    qwen3.5:35b                                        23.9 GB   qwen35moe 36.0B Q4_K_M
hf        speaches-ai/Kokoro-82M-v1.0-ONNX-int8              121 MB
hf        openai/clip-vit-large-patch14                      1.7 GB    clip FLOAT32
             model.safetensors                               1.7 GB
hf        gpustack/jina-reranker-v2-base-multilingual-GGUF   305 MB    Q8_0
             jina-reranker-v2-base-multilingual-Q8_0.gguf    305 MB
hf        BAAI/bge-reranker-v2-m3                            2.3 GB    xlm-roberta 8194ctx FLOAT32
             model.safetensors                               2.3 GB
llamacpp  unsloth/Qwen3-Next-80B-A3B-Instruct-GGUF:Q4_0     45.3 GB   Q4_0
llamacpp  unsloth/Qwen3.5-35B-A3B-GGUF:UD-Q4_K_XL           22.2 GB   UD-Q4_K_XL
llamacpp  unsloth/Qwen3.5-9B-GGUF:UD-Q4_K_XL                6.0 GB    UD-Q4_K_XL
```

The `MODEL` column width adjusts dynamically to the longest name in the result set.

#### `harbor models ls --json`

With `--json` / `-j`, each model is returned as a JSON object. The array is written to stdout.
...
```json
{
  ...
  "files": [
    { "name": "<filename>", "size": 12345678 }
  ],
  "details": {
    "family": "...",          // ollama
    "parameters": "...",      // ollama, hf gguf, llamacpp
    "quantization": "...",    // ollama, hf gguf, llamacpp
    "architecture": "...",    // hf, llamacpp
    "contextLength": 8192,    // hf gguf, llamacpp
    "dtype": "..."            // hf safetensors
  }
}
```

Not all `details` fields are present for every source — only the metadata available from the local files is included.

**Example — extract model IDs with parameters and quantization using `jq`:**

```bash
harbor models ls --json | jq -r '
  .[] | [.model, (.details.parameters // "-"), (.details.quantization // .details.dtype // "-")] | @tsv
'
```
...
qwen3.5:9b	8.95B	unknown
unsloth/Qwen3-4B-Instruct-GGUF:Q4_K_M	4.02B	Q4_K_M
BAAI/bge-reranker-v2-m3	-	FLOAT32
```

Filter to a specific source:

```bash
# llamacpp models only
harbor models ls --json | jq '[.[] | select(.source == "llamacpp") | {model, parameters: .details.parameters, quantization: .details.quantization}]'

# DMR models only
harbor models ls --source dmr --json
```

#### `harbor models pull <model>`

Download a model, routing to HuggingFace or Ollama based on the model spec unless `--source` is provided.

```bash
# Pull from HuggingFace (org/repo format)
harbor models pull unsloth/Qwen3-4B-Instruct-GGUF

# Pull from Ollama (model:tag format)
harbor models pull llama3.2:3b

# Slash-containing specs are tried against HuggingFace first
harbor models pull microsoft/Phi-4-mini-instruct-gguf
...
**Routing logic** (when `--source` is omitted):

1. Harbor queries the Hugging Face Hub API for `org/repo` (the part before an optional `:tag`, 5-second timeout).
2. If the repo is not found, the spec is passed to Ollama's `pull` command.
3. If the repo exists and publishes `.gguf` files, Harbor downloads via an ephemeral llama.cpp server into the llama.cpp cache.
4. If the repo exists but has no GGUF artifacts, Harbor downloads the repository via the Hugging Face CLI into the HF cache.

Slash-containing specs that are not on Hugging Face (for example Ollama registry names with a `/`) fall through to Ollama when the Hub API returns no match.

With `--source dmr`, Harbor delegates to Docker Model Runner. With `--source mlx`, Harbor delegates to `harbor mlx pull`, which downloads HuggingFace model repos into the local cache. With `--source omlx`, Harbor delegates to `harbor omlx pull`, which downloads HuggingFace model repos into `HARBOR_OMLX_MODEL_DIR`. With `--source ollama`, `--source hf`, or `--source llamacpp`, Harbor skips auto-detection and uses that backend directly.

#### `harbor models rm <model>`

Remove a model, searching all configured sources simultaneously and deleting every match.

```bash
# Remove by exact name (removes from whichever source(s) it matches)
harbor models rm qwen3.5:9b

# Prefix match for Ollama — removes all qwen3.5 tags
harbor models rm qwen3.5

# Remove a specific HuggingFace repo
harbor models rm unsloth/Qwen3-4B-Instruct-GGUF

# Remove only a specific quantization from the llamacpp cache
harbor models rm unsloth/Qwen3.5-9B-GGUF:UD-Q4_K_XL

# Remove from a host-backed backend
harbor models rm --source dmr ai/smollm2
harbor models rm --source omlx mlx-community/Qwen3.5-4B-4bit

# Alias
harbor models remove qwen3.5:9b
```

**Per-source removal behavior:**

- **ollama** — exact name match, or prefix match (e.g. `qwen3.5` removes `qwen3.5:latest`, `qwen3.5:8b`, etc.)
- **hf** — removes the `$HF_CACHE/hub/models--{org}--{repo}` directory; also tries loose `.gguf` files in `$HF_CACHE` and `$HF_CACHE/gguf/`
- **llamacpp** — `org/repo` removes all quantizations of that model; `org/repo:Q4_K_M` removes only files matching that quantization tag. Handles both subdirectory and flat cache layouts. Associated manifest files are also removed.
- **dmr** — delegates removal to Docker Model Runner.
- **mlx** — model removal is not supported; manage the HuggingFace cache manually.
- **omlx** — removes the matching model directory under `HARBOR_OMLX_MODEL_DIR`.

The command returns a non-zero exit code if nothing was found in any source.

#### Configuration

| Key | Description |
|-----|-------------|
| `hf.cache` | Path to HuggingFace cache root (default: `~/.cache/huggingface`) |
| `hf.ssl.cert.file` | Host path to a custom CA bundle for `harbor hf` (corporate TLS inspection); mounted at `/harbor/custom-ca.pem` and passed to `SSL_CERT_FILE` / `REQUESTS_CA_BUNDLE` when set |
| `llamacpp.cache` | Path to llama.cpp GGUF cache (default: `~/.cache/llama.cpp`) |
| `dmr.upstream.url` | Docker Model Runner endpoint used for listing models |
| `mlx.upstream.url` | mlx-lm endpoint used for listing models |
| `omlx.upstream.url` | oMLX endpoint used for listing models |
| `ollama.internal.url` | Ollama API endpoint used for listing/removing models (default: `http://ollama:11434`) |

```bash
# Read current values
harbor config get hf.cache
harbor config get llamacpp.cache
harbor config get ollama.internal.url

# Override cache locations
harbor config set hf.cache /mnt/models/huggingface
harbor config set llamacpp.cache /mnt/models/gguf

# Corporate proxy / custom CA for harbor hf downloads
harbor config set hf.ssl.cert.file /path/to/corporate-ca.pem
```
...
Manages [Docker Model Runner](./2.2.22-Backend-Docker-Model-Runner.md) — a host-native backend where Docker Model Runner runs inference on the host (Metal on macOS, GPU on Linux). Harbor exposes it to the Compose network through a Caddy reverse proxy.

```bash
# Start DMR (bootstraps host components and proxy)
harbor dmr start
...
# Pull a model from Docker Hub's ai/ namespace
harbor dmr pull ai/mistral

# Remove a model
harbor dmr rm ai/smollm2

# Stop the proxy (Docker Model Runner itself stays managed by Docker Desktop)
harbor dmr stop
```
...
|-----|---------|-------------|
| `dmr.host.port` | `34920` | Host-exposed proxy port |
| `dmr.upstream.url` | `http://model-runner.docker.internal:12434` | Docker Model Runner endpoint |
| `dmr.runner.port` | `12434` | TCP port for Docker Model Runner |
| `dmr.model` | `ai/smollm2` | Default model to auto-pull on startup |
| `dmr.manage.host` | `true` | Auto-install/enable Docker Model Runner components |
| `dmr.auto.pull` | `true` | Pull default model on startup |
| `dmr.enable.tcp` | `true` | Enable TCP endpoint on Docker Desktop |
| `dmr.api.key` | `sk-dmr` | API key for service integrations |

```bash
harbor config set dmr.model ai/mistral
harbor config set dmr.manage.host false
```

### `harbor mlx <command>`

Manages [MLX](./2.2.23-Backend-MLX.md) — Apple's MLX runtime for host-native Metal inference on Apple Silicon. Harbor manages `mlx-lm` on the host via `uv` and exposes it to the Compose network through a Caddy reverse proxy.

```bash
# Start MLX (launches mlx-lm on the host and the proxy container)
harbor mlx start

# Check mlx-lm server status
harbor mlx status
...
> Model removal is not supported through `harbor mlx rm`. Manage the HuggingFace cache manually.

#### Configuration

| Key | Default | Description |
|-----|---------|-------------|
| `mlx.host.port` | `34930` | Host-exposed proxy port |
| `mlx.upstream.url` | `http://host.docker.internal:8095` | mlx-lm endpoint for the proxy |
| `mlx.runner.port` | `8095` | Port for the host mlx-lm server |
| `mlx.model` | `mlx-community/Qwen3.5-4B-4bit` | Default model name for launch and integrations |
| `mlx.hf.path` | `mlx-community/Qwen3.5-4B-4bit` | HuggingFace repo passed to `mlx-lm` on the host |
| `mlx.manage.host` | `true` | Auto-start mlx-lm on the host |
| `mlx.workspace` | `./services/mlx` | Working directory for mlx-lm |
...
Manages [oMLX](./2.2.24-Backend-oMLX.md) — a host-native Apple Silicon MLX inference server with OpenAI-compatible APIs, continuous batching, and paged SSD KV caching. Harbor manages `omlx serve` on the host via `uv` and exposes it to the Compose network through a Caddy reverse proxy.

```bash
# Start oMLX (launches oMLX on the host and the proxy container)
harbor omlx start
...
# Pull a model from HuggingFace into the oMLX model directory
harbor omlx pull mlx-community/Qwen3.5-4B-4bit

# View oMLX logs
harbor omlx logs

# Remove a local oMLX model directory
harbor omlx rm mlx-community/Qwen3.5-4B-4bit
...
| Key | Default | Description |
|-----|---------|-------------|
| `omlx.host.port` | `34940` | Host-exposed proxy port |
| `omlx.upstream.url` | `http://host.docker.internal:8096` | oMLX endpoint for the proxy |
| `omlx.runner.port` | `8096` | Port for the host oMLX server |
| `omlx.model` | `Qwen3.5-4B-4bit` | Default model for launch and integrations |
| `omlx.hf.path` | `mlx-community/Qwen3.5-4B-4bit` | HuggingFace repo to download |
| `omlx.base.path` | `./services/omlx` | Base path passed to `omlx serve --base-path` |
| `omlx.model.dir` | `./services/omlx/models` | Directory containing MLX-format model folders |
| `omlx.cache.dir` | `./services/omlx/cache` | Paged SSD KV cache directory |
| `omlx.api.key` | *(empty)* | Optional API key for host runner and `omlx_curl` |
| `omlx.extra.args` | *(empty)* | Extra arguments appended to `omlx serve` |
| `omlx.manage.host` | `true` | Auto-start oMLX on the host |
| `omlx.workspace` | `./services/omlx` | Working directory for oMLX (`uv` project root) |
...
Runs CLI tasks specific to managing the `marinara` service (Marinara Engine). See [the service doc](./2.1.17-Frontend-Marinara.md) for the full configuration reference.

#### `harbor marinara version [tag]`

Get/set the Marinara Engine docker image tag.

```bash
harbor marinara version
harbor marinara version 1.5.0
```

#### `harbor marinara secret [secret]`

Get/Set `ADMIN_SECRET`, required for Marinara's privileged APIs (admin cleanup, backups, bulk import, sidecar management).

```bash
harbor marinara secret
harbor marinara secret sk-203948
```

#### `harbor marinara key [key]`

Get/Set `ENCRYPTION_KEY`, the AES key Marinara uses to encrypt stored provider API keys at rest.

```bash
harbor marinara key

# Generate and set a new random key
harbor marinara key generate

# Rotating an existing key makes previously stored data unreadable;
# --force is required to overwrite a key that is already set
harbor marinara key generate --force
```

#### `harbor marinara auth <user> <pass>`

Set `BASIC_AUTH_USER`/`BASIC_AUTH_PASS`, required before exposing Marinara beyond loopback/Docker traffic. With no arguments, prints the configured user (never the password). Setting a user/pass overrides `ip-allowlist` below for general access control — pass empty strings to clear both and fall back to the allowlist instead.

```bash
harbor marinara auth
harbor marinara auth harbor 's3cr3t'

# Clear Basic Auth (e.g. to use ip-allowlist as the access-control gate instead)
harbor marinara auth "" ""
```

#### `harbor marinara trusted-hosts [hosts]`

Get/Set `TRUSTED_HOSTS`, extra public or reverse-proxy hostnames Marinara will answer to (DNS-rebinding protection on the `Host` header). Only needed when Marinara is reached by hostname rather than raw IP. Takes the whole comma-separated value as one argument.

```bash
harbor marinara trusted-hosts
harbor marinara trusted-hosts "chat.example.com"
```

#### `harbor marinara csrf-origins [origins]`

Get/Set `CSRF_TRUSTED_ORIGINS`, extra browser origins allowed to submit form/API changes without CSRF rejection. Takes the whole comma-separated value as one argument.

```bash
harbor marinara csrf-origins
harbor marinara csrf-origins "https://chat.example.com"
```

#### `harbor marinara ip-allowlist [ips]`

Get/Set `IP_ALLOWLIST`, comma-separated IPs or CIDR ranges always allowed in, bypassing auth (loopback is always allowed regardless). `IP_ALLOWLIST_ENABLED` defaults to `true` upstream, so setting this alone activates it — no separate enable step. Takes the whole comma-separated value as one argument; re-running replaces rather than appends. Note this only governs *general* access — if Basic Auth (`harbor marinara auth`) is also set, Basic Auth takes priority.

```bash
harbor marinara ip-allowlist
harbor marinara ip-allowlist "192.168.1.0/24,10.0.0.5"
```

#### `harbor marinara log [level]`

Get/Set the Marinara server log level (`debug`, `info`, `warn`, `error`).

```bash
harbor marinara log
harbor marinara log debug
```

#### `harbor marinara llama [image]`

Get/Set `HARBOR_MARINARA_SYSTEM_LLAMA_IMAGE`, the llama.cpp server image a `marinara-llama-init` sidecar extracts `llama-server` from onto Marinara's `PATH`, backing the **System llama-server** option (and the default *Auto* preference) in Marinara's Local AI Model panel. Takes effect on the next `harbor up marinara`.

```bash
harbor marinara llama
harbor marinara llama ghcr.io/ggml-org/llama.cpp:server-cuda
```

#### `harbor marinara local [on|off]`

Get/Set whether Marinara is allowed to reach AI provider URLs on private/LAN/Docker addresses — the gate that must be open for Marinara to reach another Harbor backend service (`dmr`, `llamacpp`, `mlx`, `ollama`, `omlx`).

```bash
harbor marinara local
harbor marinara local on
```

#### `harbor marinara workspace`

Open the Marinara workspace directory (`services/marinara/`) in the system file manager.
...
Runs CLI tasks specific to managing the `swarmui` service (SwarmUI). See [the service doc](./2.1.18-Frontend-SwarmUI.md) for the full configuration reference.

SwarmUI itself reads no environment variables for configuration — everything is a CLI arg or stored in `Data/Settings.fds` via the UI — so this command only manages the handful of settings that affect how Harbor builds and mounts the container.

#### `harbor swarmui args [args]`

Get/Set extra CLI args appended to SwarmUI's launch script (e.g. `--lock_settings`).

```bash
harbor swarmui args
harbor swarmui args "--lock_settings"
```

#### `harbor swarmui models [path]`

Get/Set the host path mounted as SwarmUI's `Models` folder. Defaults to `./services/swarmui/models`; repoint it at `./services/comfyui/workspace/storage/stable_diffusion/models` to share checkpoints with the `comfyui` service instead of downloading duplicates.

```bash
harbor swarmui models
harbor swarmui models ./services/comfyui/workspace/storage/stable_diffusion/models
```

#### `harbor swarmui output`

Open the folder containing SwarmUI's generated output in the system file manager.

```bash
harbor swarmui output
```

#### `harbor swarmui workspace`

Open the SwarmUI workspace directory (`Data`, `dlbackend`, `extensions`) in the system file manager.
...
# Add a new URL
harbor openai urls add <url>

# Remove a URL
harbor openai urls rm <url>
# Remove by index (zero-based)
harbor openai urls rm 0
...
See the [service guide](./2.3.4-Satellite&colon-Plandex.md) for some additional details on the Plandex service setup.
...
Get/Set the model currently configured to run. See a more detailed guide in the [`mistralrs` service guide](./2.2.6-Backend&colon-mistral.rs.md).
...
See [Fabric Documentation](https://github.com/danielmiessler/fabric) for pattern details.
...
See [Aider Documentation](https://aider.chat/) for detailed usage.
...
### `harbor facts <command>`

Run the facts CLI against the current directory in a Harbor container.
...
Run the mi agent CLI against the current directory in a Harbor container.
...
# One-shot prompt
harbor mi -p "summarize this project"

# Review piped input
git diff | harbor mi -p "review this diff"
```

### `harbor npcsh <command>`

Run the npcsh CLI/TUI against the current directory in a Harbor container.

```bash
# Interactive npcsh shell
harbor npcsh
...
#### `harbor comfyui image`

Get/set the ComfyUI image repository.
...
harbor comfyui image yanwk/comfyui-boot
```

#### Deprecated: `harbor comfyui user` / `password` / `auth` / `workspace sync`

These commands were features of the old `ai-dock/comfyui` image and are now no-ops. Upstream ComfyUI has no built-in authentication — use `harbor tunnel comfyui` or front it with `traefik` and an auth middleware to expose it safely. The `comfyui-boot` image persists everything in the workspace, so `workspace sync` is no longer needed.
...
See [AIChat Documentation](https://github.com/sigoden/aichat) for details.
...
See [Nexa Documentation](https://github.com/NexaAI/nexa-sdk) for details.
...
See [Repopack Documentation](https://github.com/yamadashy/repopack) for details.
...
See [K6 Documentation](https://k6.io/docs/) for test script details.
...
See [Promptfoo Documentation](https://www.promptfoo.dev/) for details.
...
See [GPTme Documentation](https://github.com/ErikBjare/gptme) for details.
...
See [PhotoPrism CLI Documentation](https://docs.photoprism.app/user-guide/ai/cli/) for available commands.
...
See [lm-evaluation-harness](https://github.com/EleutherAI/lm-evaluation-harness) for task details.
...
Creates a symlink to the `harbor.sh` script in the user's home bin directory. This allows you to run the script from any directory. Safe to re-run: existing symlinks are replaced if the target path or CLI name changed. Supports bash, zsh, and fish shells -- adds the appropriate PATH configuration to your shell profile automatically.
...
An antipode to `harbor link`. Removes previously added symlinks and cleans up PATH entries from shell profiles (bash, zsh, fish). Note that this uses _current_ links configuration, so if it was changed since the link was added, it might not work as expected.

```bash
# Removes the symlink(s) and shell profile PATH entries
harbor unlink
```
...
Manage custom host volume mounts for Harbor services. Volumes added here are injected into the corresponding containers at startup, in addition to the service's default volumes.

```bash
# Show all services with custom volumes
harbor volumes ls

# Show volumes for a specific service
harbor volumes ls ollama

# Add a volume mount (docker-style source:dest notation)
harbor volumes add ollama /data/models:/root/.ollama
harbor volumes add ollama /certs/ca.pem:/etc/ssl/certs/ca.pem

# Remove a volume by index (zero-based)
harbor volumes rm ollama 0

# Remove all custom volumes for a service
harbor volumes clear ollama
```

Volume configuration is stored in the `.env` file as semicolon-separated entries under `HARBOR_<SERVICE>_VOLUMES`. You can also manage them directly via `harbor config`:

```bash
harbor config get ollama.volumes
harbor config set ollama.volumes "/data/models:/root/.ollama;/certs/ca.pem:/etc/ssl/certs/ca.pem"
```

Volumes work with any service name, including sub-services like `dify-api` or `ollama-init`. Custom mounts are merged into the generated Compose file at startup; run `harbor up` (or restart the service) after adding or changing volumes.
...
Harbor assembles its defaults from two places, in this order:

1. `profiles/default.env` — global options
2. `services/<name>/default.env` — that service's `HARBOR_<NAME>_*` options

Both use the same syntax, and every command that reads defaults (`harbor config
reset`, `harbor config update`, `harbor profile merge`, `harbor doctor`, and the
initial `.env` creation) reads the concatenation of all of them. New services put
their defaults in `services/<name>/default.env`.

#### `harbor config reset`

Resets the current `.env` configuration to its original form, based on the default
env files (`profiles/default.env` plus every `services/<name>/default.env`).
...
Will merge the default env files (`profiles/default.env` plus every
`services/<name>/default.env`) with the current local `.env` in order to add new
configuration options. Typically used after updating Harbor when new variables are
added. Most likely, you won't need to run this manually, as it's done automatically
after `harbor update`.

This process won't overwrite user-defined variables, only add new ones.

```bash
# Merge the default env files with the current .env
harbor config update
```

#### `harbor config search <query>`

Searches Harbor configuration keys (case-insensitive) and prints matching key-value pairs.

```bash
# Search for all config keys containing "webui"
harbor config search webui

# Search for port-related settings
...
Allows managing service-specific environment variables directly through `harbor config`. This is functionally equivalent to [`harbor env`](#harbor-env).

Service detection works by checking if `services/<service>/override.env` exists.

```bash
# List all env vars for a service
harbor config ollama ls

# Get a specific service env var
harbor config ollama get OLLAMA_NUM_PARALLEL

# Set a service env var
harbor config ollama set OLLAMA_NUM_PARALLEL 4

# Shorthand get (no subcommand needed)
harbor config ollama OLLAMA_NUM_PARALLEL

# Shorthand set (no subcommand needed)
harbor config ollama OLLAMA_NUM_PARALLEL 4

# Search within service env vars
harbor config ollama search PARALLEL
```

Note: unlike global `harbor config` keys, service env vars are NOT prefixed with `HARBOR_` — they use the service's native variable names directly. See the corresponding `services/<service>/override.env` file for available variables.

> See also: [`harbor env`](#harbor-env) — a dedicated shortcut for the same functionality.
...
> **Tip:** You can also use `harbor config <service>` for the same functionality. See [`harbor config <service>`](#harbor-config-service).
...
# or explicitly
harbor env <service> get <key>

# Set a new env var
harbor env <service> <key> <value>

# Remove an env var
harbor env <service> unset <key>
```
...
Updates Harbor to the latest release. Fetches the latest tag from GitHub, checks out the new version, merges new config keys into your `.env`, and runs any config migrations.

Per-service `override.env` files (set via `harbor env`) are preserved across the update via git stash. If running services are detected, a warning is shown to restart them after the update.
...
> [!TIP]
> After updating, the old and new version numbers are displayed to confirm the update applied.
...
Walks all `CACHE` and singular `WORKSPACE` directories from [`harbor config ls`](#harbor-config-list) and prints sizes for the directories that currently exist, additionally displays a size for `$(harbor home)` directory. Multi-path entries such as `OPENCODE_WORKSPACES` are skipped.
...
# Run installed host coding tools through harbor launch against a live backend
harbor dev launch-live-smoke --backend ollama --model qwen3.5:4b

# Regenerate the docs (Boost references are rendered inside the boost container,
# run "harbor build boost" first so they don't come from a stale image)
harbor build boost
harbor dev docs

# Verify the generated Boost references are up to date, without writing them
harbor dev docs --check
```
