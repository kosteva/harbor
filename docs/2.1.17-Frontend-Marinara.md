...
HARBOR_MARINARA_HOST_PORT          35080
...
# llama.cpp server image whose llama-server binary is extracted onto
# Marinara's PATH for its "System llama-server" local runtime. See
# "Local AI Model Runtime" below.
HARBOR_MARINARA_SYSTEM_LLAMA_IMAGE ghcr.io/ggml-org/llama.cpp:server
```
...
harbor marinara llama [image]        # Get/set the image providing the System llama-server runtime
...
Marinara's **Local AI Model** panel can also run its own llama.cpp process inside the
container ("sidecar") instead of talking to an external backend — pick a GGUF, choose a
runtime, and it launches `llama-server` for you. The runtime dropdown offers *Auto* (default)
or a specific one, including **System llama-server** (a binary already on the container's
`PATH`, as opposed to one Marinara downloads itself).

Harbor provisions that binary automatically: a `marinara-llama-init` sidecar container extracts
`llama-server` from `HARBOR_MARINARA_SYSTEM_LLAMA_IMAGE` into a shared volume before Marinara
starts, and a wrapper script puts it on `PATH` at container start. `MARINARA_SIDECAR_USE_SYSTEM_LLAMA=true`
is also set, so even the default *Auto* preference picks this build up instead of downloading a
release from GitHub the first time a local model is started.

To pin a different llama.cpp build (a specific version, or a CUDA/Vulkan variant):
```bash
harbor marinara llama ghcr.io/ggml-org/llama.cpp:server-cuda
harbor down marinara && harbor up marinara
```
The image tag only takes effect on the next `harbor up marinara`, since that's when
`marinara-llama-init` re-runs.

GPU passthrough for a CUDA/ROCm image tag is automatic, using the same NVIDIA/CDI
auto-detection Harbor applies to `llamacpp`, `ollama`, and other GPU-capable services — no
extra config needed beyond having `nvidia-container-toolkit` (or a CDI spec) set up on the
host. Restart with `harbor down marinara && harbor up marinara` after switching to a GPU image
tag so the capability is picked up.

This is unrelated to [Using a Harbor Backend](#using-a-harbor-backend) above — the sidecar
spawns its own `llama-server` process inside the Marinara container, while a Harbor backend
(`llamacpp`, `ollama`, etc.) is a separate container reached over HTTP.
